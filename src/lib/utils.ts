...
export class AppUtils {
    ...
    static compareObjectID(obj1: Types.ObjectId, obj2: Types.ObjectId): boolean {
        return obj1.toString() === obj2.toString();
    }

    static Next5Mins = (): Date => {
    ...
    static hashIdentityNumber = (identity_number: string) => {
        return crypto.createHash("sha256").update(identity_number).digest("hex");
    }

    static matchAccNameInDb(
    ...
    static rateLimitMiddlewareHandler(_req: Request, res: Response, _next: NextFunction, _: Options) {
        return res.status(429).json({ error: true, message: "Too many requests" });
    }

    static resolveLocation(location: string): LocationType {
    ...
    static querySafeHavenBankCodes(q: RegExp) {
    ...
    static AdPaymentsMap = {
        weekly: 7_000,
        monthly: 22_000
    }

    static RmOnCompleteOpts = { age: 36, count: 1000 };

    static RmOnFailOpts = { age: 24 * 3600 };
}
